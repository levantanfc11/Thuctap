...
class Process(object):
    def __init__(self):
        ...
        self.freqs = []
        self.t0 = time.time()
        self.bpm = 0 #chi sonhip tim
        self.fd = FaceDetection()
        self.bpms = [] 
        self.peaks = []
        self.sbp = 0 #huyet ap tam thu
        self.sbps = []
        self.dbp = 0 #huyet ap tam truong
        self.dbps = []
        self.bn = 0 #brightness (do sang)
        self.bns = []
        self.breath = 0 #chi so nhip tho
        self.breaths = []
    ...
    def convert_gray_color(self, frame):
    ...
    def run(self):
        ...
        if L == self.buffer_size:
            ...
            #calculating SBP and DBP (Huyet ap tam thu - Huyet ap tam truong)
            I_Tsys = min(processed)
            I_Tdia = max(processed)
            I_S = I_Tdia - I_Tsys

            self.sbp = -0.599*I_Tsys - 0.656*I_S + 165.942
            self.sbps.append(self.sbp)
            self.dbp = -0.212*I_Tdia - 0.251*I_S + 96.211
            self.dbps.append(self.dbp)

            #convert RGB color to GRAY COLOR from camera and calculate average gray value of 2 ROIS
            gray_frame_1 = self.convert_gray_color(ROI1)
            gray_frame_2 = self.convert_gray_color(ROI2)
            gray_frame_avarage = (gray_frame_1.mean() + gray_frame_2.mean()) / 2

            #calculate brightness
            self.bn = gray_frame_avarage
            self.bns.append(self.bn)
            ...
            #calculate respiratory rate by using heart rate (HRRR)
            hr = (self.bpm - 60) / 60
            self.breath = 15.14 + 1.88 * hr + 4.17 * hr * hr
            self.breaths.append(self.breath)
            #Link cong thuc: https://www.nature.com/articles/s41746-021-00493-6
