...
class GUI(QMainWindow, QThread):
    ...
    def initUI(self):
        ...
        self.lblHR.setGeometry(870,20,300,40)
        self.lblHR.setFont(font)
        self.lblHR.setText("Frequency: ")
        
        self.lblHR2 = QLabel(self) #label to show stable HR
        self.lblHR2.setGeometry(870,50,300,40)
        self.lblHR2.setFont(font)
        self.lblHR2.setText("Heart rate: ")

        self.lblHR3 = QLabel(self) #label to show stable BP
        self.lblHR3.setGeometry(870,70,300,40)
        self.lblHR3.setFont(font)
        self.lblHR3.setText("BP: ")

        self.lblHR4 = QLabel(self) #label to show stable BN
        self.lblHR4.setGeometry(870,90,300,40)
        self.lblHR4.setFont(font)
        self.lblHR4.setText("Brightness: ")

        self.lblHR5 = QLabel(self) #label to show stable BR
        self.lblHR5.setGeometry(870,110,300,40)
        self.lblHR5.setFont(font)
        self.lblHR5.setText("Breath: ")

        self.lblHR6 = QLabel(self) #label to show stable ST
        self.lblHR6.setGeometry(870,130,300,40)
        self.lblHR6.setFont(font)
        self.lblHR6.setText("Stress: ")
        ...
        self.signal_Plt.move(700,220)
        self.signal_Plt.resize(550,192)
        self.signal_Plt.setLabel('bottom', "Signal") 
        
        self.fft_Plt = pg.PlotWidget(self)
        
        self.fft_Plt.move(700,425)
        self.fft_Plt.resize(550,192)
    ...
    def main_loop(self):
        ...
        self.sbp = self.process.sbp #get the sbp change over the time
        self.dbp = self.process.dbp #get the dbp change over the time
        self.bn = self.process.bn #get the brightness change over the time
        self.breath = self.process.breath #get the breath change over the time
        ...
        #calculate stress index using heart rate and breathe
        min_stress = 0.0
        max_stress = 10.0
        self.stress = 0.6 * np.mean(self.process.bpms) + 0.4 * np.mean(self.process.breaths)
        if self.stress >=0 and self.stress <=100:
            rescaled_stress = (self.stress - min_stress) / (max_stress - min_stress)
        else:
            rescaled_stress = 0
        ...
        if self.process.bpms.__len__() >50:
            if(max(self.process.bpms-np.mean(self.process.bpms))<5): #show HR if it is stable -the change is not over 5 bpm- for 3s
                ...
                self.lblHR3.setText("Blood Pressure: " + str(float("{:.2f}".format(np.mean(self.process.sbps)))) + "/" + str(float("{:.2f}".format(np.mean(self.process.dbps)))) + "mmHg")
                self.lblHR4.setText("Brightness: " + str(float("{:.2f}".format(np.mean(self.process.bns)))))
                self.lblHR5.setText("Breath: " + str(float("{:.2f}".format(np.mean(self.process.breaths)))) + " brpm")
                self.lblHR6.setText("Stress: " + str(float("{:.2f}".format(rescaled_stress))) + "/10.0")
