...
def resize(*args, **kwargs): #Thay đổi kích thước khung hình
    return cv2.resize(*args, **kwargs)

def moveWindow(*args,**kwargs): #Di chuyển khung hình
    return

def imshow(*args,**kwargs): #Hiển thị một hình ảnh hoặc khung hình lên cửa sổ
    return cv2.imshow(*args,**kwargs)
    
def destroyWindow(*args,**kwargs): #Huỷ khung hình
    return cv2.destroyWindow(*args,**kwargs)

def waitKey(*args,**kwargs): #Chờ phản hồi từ người dùng
    return cv2.waitKey(*args,**kwargs)

    
"""
The rest of this file defines some GUI plotting functionality. There are plenty
of other ways to do simple x-y data plots in python, b do real-time data plotting and handle user interaction.ut this application uses 
cv2.imshow to
...
def combine(left, right):
    ...
    comb = np.zeros(tuple(shape),left.dtype) #Tạo ra một mảng numpy, tất cả phân từ = 0, tuple(shape) đại diện cho kích thước của mảng mới
...
def peakdet(v, delta, x = None):
    ...
    if x is None:
        x = np.arange(len(v)) #Tạo ra một mảng có giá trị từ 0 -> len(v) - 1
...
def plotXY(data,size = (480,640),margin = 25,name = "data",labels=[], skip = [],
    ...
    if isinstance(bg,np.ndarray):
        ...
        if len(bg.shape) == 3:
            r = combine(bg[:,:,0],z[:,:,0]) #Kết hợp 2 mảng
            g = combine(bg[:,:,1],z[:,:,1])
            b = combine(bg[:,:,2],z[:,:,2])
        else:
            r = combine(bg,z[:,:,0])
            g = combine(bg,z[:,:,1])
            b = combine(bg,z[:,:,2])
        z = cv2.merge([r,g,b])[:,:-wd,] #Kết hợp 3 mảng, '-wd' lấy tất cả trừ cột cuối cung
